...
Arvore *inserir(int valor, Arvore *raiz) {
    if(raiz) {
        ...
        // Descobri se está balanceado
        if(raiz->fb == -2) { // Rotação para esquerda
            // Descobrir se a rotação é simples 
            if(raiz->fb < 0 && raiz->dir->fb < 0) {
                raiz = rodaEsquerda(raiz);
            } else {
                // Rotação dupla 
                // 1 - Rodar o filho para a direita
                raiz->dir = rodaDireita(raiz->dir);
                // 2 - Rodar o pai paa esquerda
                raiz = rodaEsquerda(raiz);
            }
        }
        else if(raiz->fb == 2) { // Rotação para a direita
            // Descobrindo se a rotação é simples
