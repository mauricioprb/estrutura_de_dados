...
void moda(Celula *lista, FILE *procuradorSaida){
    Celula *p1, *p2;
    int valor, valorModa;
    int qtdValor, qtdValorModa = 0;

    for(p1 = lista; p1; p1 = p1->prox){
        if(valor == p1->valorGlicemia){
            continue;
        }
        valor = p1->valorGlicemia;
        qtdValor = 0;
        if(valor != valorModa){
            for(p2 = lista; p2; p2 = p2->prox){
                if (valor == p2->valorGlicemia){
                    qtdValor++;
                }
                if(valor < p2->valorGlicemia){
